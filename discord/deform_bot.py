...
def distort_image(fname, args, png: bool = False):
    ...
    if (("u" not in args) and (not png)):  # disable-compression flag
        # no '-colorspace RGB'
    ...
    for e in args:
        ...
        if e.startswith('h'): # hue-flag
            try:
                ...
                continue
            if 1 <= cast_int <= 360:
                build_str += f" -modulate 100,100,{cast_int} "
            continue
        if e.startswith('x'): # pixel-spread-flag
            try:
                cast_int = int(e[1:4])
            except:
                arg_error_flag = True
    ...
    if anaglyph:
        ...
        stereocmd = "magick composite " + os.path.join("results", f"{fname}") + " " + os.path.join(
...
async def help(ctx):
    ...
    help_embed = discord.Embed(
        description="[Website](https://bjarne.dev)\n[Github](https://github.com/bj4rnee/DeformBot)\n[Twitter](https://twitter.com/DeformBot)" + \
            "\n\n**Commands:**\n`help`:  Shows this help message\n`deform`:  Distort an attached image\nYou can also react to an image with 🤖 to quickly deform it." + \
            helpstr_args + helpstr_usage, color=rand_color)
